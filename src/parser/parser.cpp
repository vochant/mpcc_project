...
std::shared_ptr<Node> Parser::parse_function_creation() {
	if (_current->type != Token::Type::Function && _current->type != Token::Type::Constructor && _current->type != Token::Type::Destructor) {
		unhandled_compiler_error();
		return make_error();
	}
	...
	std::string _name;
	parse_token();
	if (funcType == Token::Type::Function) {
		if (_current->type != Token::Type::Identifier) {
			format_error();
			return make_error();
		}
	...
	if (funcType == Token::Type::Constructor) {
		_name = "__spec_constructor" + std::to_string(_obj->args.size());
	}
...
std::shared_ptr<Node> Parser::parse_class() {
...
std::shared_ptr<Node> Parser::parse_enumerate() {
...
std::shared_ptr<Node> Parser::parse_named_constant() {
	if (_current->type == Token::Type::Object) {
		return std::make_shared<ConstantNode>(std::make_shared<Environment>());
	}
